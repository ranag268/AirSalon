...
if ( ! function_exists( 'woocommerce_template_loop_product_title' ) ) {
	...
	function woocommerce_template_loop_product_title() {
		echo '<h2 class="title text-secondary">' . get_the_title() . '</h2>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
...
if ( ! function_exists( 'woocommerce_template_loop_product_link_open' ) ) {
	...
	function woocommerce_template_loop_product_link_open() {
		...
		echo '<div class="p-3 p-md-5"><a href="' . esc_url( $link ) . '">';
	}
}
				...
				'class'      => implode(
					...
					array_filter(
						array(
							...
							$product->is_purchasable() && $product->is_in_stock() ? 'btn btn-custom text-small d-block p-1' : '',
...
if ( ! function_exists( 'woocommerce_get_product_thumbnail' ) ) {
	...
	function woocommerce_get_product_thumbnail( $size = 'woocommerce_thumbnail', $deprecated1 = 0, $deprecated2 = 0 ) {
		...
		return "<img src='$image[0]' class='lazy' height='$image[2]' width='$image[1]' /> ";
	}
}

if ( ! function_exists( 'woocommerce_grants_filter' ) )
{
	function woocommerce_grants_filter()
	{
		dynamic_sidebar( 'shop-searcher' );
	}
}
	...
	$image             = wp_get_attachment_image(
		...
		apply_filters(
			...
			array(
				...
				'class'                   => esc_attr( $main_image ? 'm-auto wp-post-image' : '' ),
			),
			$attachment_id,
			$image_size,
			$main_image
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		switch ( $args['type'] ) {
			...
			case 'state':
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="form-control" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ? $args['placeholder'] : esc_html__( 'Select an option&hellip;', 'woocommerce' ) ) . '"  data-input-classes="' . esc_attr( implode( ' ', $args['input_class'] ) ) . '">
					...
					$field .= '<input type="text" class="form-control" value="' . esc_attr( $value ) . '"  placeholder="' . esc_attr( $args['placeholder'] ) . '" name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" ' . implode( ' ', $custom_attributes ) . ' data-input-classes="' . esc_attr( implode( ' ', $args['input_class'] ) ) . '"/>';
			...
			case 'url':
			case 'tel':
				$field .= '<input type="' . esc_attr( $args['type'] ) . '" class="form-control" name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" placeholder="' . esc_attr( $args['placeholder'] ) . '"  value="' . esc_attr( $value ) . '" ' . implode( ' ', $custom_attributes ) . ' />';
		...
		if ( ! empty( $field ) ) {
			...
			if ( $args['label'] && 'checkbox' !== $args['type'] ) {
				$field_html .= '<label for="' . esc_attr( $label_id ) . '" style="color: #bf8a2e;">' . $args['label'] . $required . '</label>';
			}
